...
--%>
							...
							<div class="col-lg-3">
								<s:message code="label.enterexamdate" var="date" />
								<sf:input path="exameDate"
									cssClass="form-control border-left-right"
									placeholder="${date}" id="datepicker" readonly="true"/>
							</div>
							<%-- <div class="col-lg-3">
										...
										<c:forEach items="${list}" var="timetable" varStatus="ct">
											<tr>
												<td class="text-center"><input class="checkbox"
													name="chk_1" type="checkbox" value="${timetable.id}"></td>
												<td class="text-center"><c:out
														value="${(form.pageSize * (form.pageNo-1))+ct.index+1}" />
													<c:set var="index"
														value="${(form.pageSize * (form.pageNo-1))+ct.index+1}" />
												</td>
												<td><fmt:formatDate value="${timetable.date}" type="date"
														pattern="dd-MM-yyyy" /></td>
												<td class="text-center">${timetable.exameTime}</td>
												...
												<td class="text-center"><c:url var="editUrl"
													...
													name="operation" href="${editUrl}${timetable.id}"><i
